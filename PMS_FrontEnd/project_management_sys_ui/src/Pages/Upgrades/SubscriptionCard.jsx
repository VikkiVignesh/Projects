...
import { createPayment } from "../../redux/payment/payActions";

const SubscriptionCard = ({ title, price, features, planType, current }) => {
  const dispatch = useDispatch();
  const jwt = localStorage.getItem("jwt");

  console.log("JWT subCard",jwt)
  const handleUpgrade = () => {
    if (current) return; // prevent re-subscribing to current plan
    dispatch(createPayment({planType,jwt:jwt}));
  };
    ...
    <div className="relative w-72 rounded-2xl p-6 backdrop-blur-md bg-[#0b1120]/90 border border-white/20 shadow-cyan-500/50 shadow-lg hover:scale-105 transition-transform duration-300 overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col justify-between h-full">
        ...
        <button
          ...
          className={`cursor-pointer mt-6 py-2 w-full font-semibold rounded-lg shadow-lg transition-transform duration-300 ${
            current
